...
int update(String sql, Object[] arge)
...
public class JDBCTemplateDAO {
	...
	public void write(final SpringBbsDTO springBbsDTO){
		...
		final로 선언하지 않으면 에러가 발생한다. (Java문법)
		 */
		template.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException{
				...
				구성하기 때문이다.
				또한, nextval은 한문장에서 여러번 사용하더라도 항상 같은
				시퀀스를 반환한다.
				 */
	...
	public SpringBbsDTO view(String idx) {
		...
		try {
			/*
			하나의 결과를 반환하는 select계열의 쿼리문을 실행할 때는
			queryForObject()메소드를 사용한다. 단, 반환결과가 0이거나
			2이상이면 예외를 발생시키므로 반드시 예외처리를 하는것이
			좋다.
			 */
	...
	public int password(String idx, String pass) {
		...
		try {
			...
			/*
			일변번호는 시퀀스를 사용하므로 반드시 1이상의 값을
			가지게 된다. 따라서 0이하의 값이 반환된다면 패스워드
			검증 실패라 판단할 수 있다.
			 */
			retNum = dto.getIdx();
		}
	...
	public void reply(final SpringBbsDTO dto) {
		...
		/*
		답변글의 경우라도 새로운 게시물이므로 일련번호를 새로운 시퀀스를
		입력해야 한다.
